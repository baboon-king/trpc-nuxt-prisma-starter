...
const searchByPage = publicProcedure
  ...
  .query(async ({ ctx, input }) => {
    ...
    const [data, count] = await ctx.prisma.$transaction([
      ctx.prisma.schemeTheme.findMany({
        ...paginationGen(input.pager),
        where,
      }),
      ctx.prisma.schemeTheme.count({
        where,
      }),
    ]);
...
const create = publicProcedure
  ...
  .mutation(({ ctx, input }) => {
    return ctx.prisma.schemeTheme.create({
      data: {
        name: input.name,
      },
    });
...
const update = publicProcedure
  ...
  .mutation(({ ctx, input }) => {
    return ctx.prisma.schemeTheme.update({
      where: {
        id: input.id,
      },
      data: input,
...
const deleteByIds = publicProcedure.input(cuidArrayZod).mutation(({ ctx, input }) => {
  return ctx.prisma.schemeTheme.deleteMany({
    where: {
      id: {
        in: input,
      },
...
const switchPublished = publicProcedure.input(switchPublishedZod).mutation(({ ctx, input }) => {
  return ctx.prisma.schemeTheme.update(switchPublishedQueryGen(input));
});

const published = publicProcedure.query(({ ctx }) => {
  return ctx.prisma.schemeTheme.findMany({
    where: {
      published: true,
    },
  });
...
export const schemeTheme = createTRPCRouter({
  published,
  searchByPage,
  update,
  create,
